...
- **Fix ALL CI failures, even pre-existing ones unrelated to the PR.** If a CI job was already failing on `main` before this PR, it must still be fixed in this PR. A failing CI pipeline blocks deployment regardless of cause. When you find a pre-existing CI failure: (1) identify the root cause by reading the failure logs (`gh run view <RUN_ID> --log-failed`), (2) fix it directly in the current branch, (3) note it in your review as "Fixed pre-existing CI failure: [description]". Do not accept the rationalization that a failure is "pre-existing and unrelated" — all red must be green before merge.
